...
def main():
    ...
    while True:
        try:
            ...
            result = PrettyTable()
            result.field_names = ["".join(calculate(formula_list))]
            print(result)
        except EOFError:
            print()
            sys.exit()
    ...
    if formula[0] in ("a", "s", "c", "t"):
        ...
        for i in range(len(formula)):
            if formula[i] == "[":
                ...
                calculated_2 = trig_operate(formula)
        ...
        calculated_2 = reg_operate(formula)
        if calculated_2 == "Error":
            return "Error"
        return calculated_2


def reg_operate(formula):
